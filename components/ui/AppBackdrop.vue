  ...
  <Teleport to="body">
    <Transition name="app-backdrop">
      <div
        ...
        :class="['app-backdrop', uiStore.overlayAboveHeader ? 'app-backdrop--high' : 'app-backdrop--low']"
...
.app-backdrop {
  ...
  &--low  { z-index: #{$z-dropdown - 1}; }

  // search / auth / sidebar must cover the header too
  // → sit between header (10) and the modal (20 / 30)
  &--high { z-index: #{$z-modal-overlay - 1}; }
}

.app-backdrop-enter-active,
.app-backdrop-leave-active { transition: opacity 0.2s ease; }
.app-backdrop-enter-from,
.app-backdrop-leave-to     { opacity: 0; }
</style>
